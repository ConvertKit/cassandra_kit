...
module CassandraKit
  module Schema
    ...
    #   {CassandraKit::Metal::Keyspace} in a future version of CassandraKit
    #
    class Keyspace
      ...
      def create!(options = {})
        ...
        replication_options_strs = options[:replication].map do |name, value|
          "'#{name}': #{CassandraKit::Type.quote(value)}"
        end
      ...
      def drop!
        keyspace.execute("DROP KEYSPACE #{keyspace.name}").tap do
          ...
          #   session.keyspace # => "cassandra_kit_test"
          #   session.execute("DROP KEYSPACE cassandra_kit_test")
          #   session.keyspace # => nil
          # This causes problems in the specs where we drop the test keyspace
          # and recreate it. CassandraKit::Record.connection.client's keyspace will
