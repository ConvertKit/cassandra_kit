...
module CassandraKit
  module Record
    #
    # `CassandraKit::Record` implementations define their own schema in their class
    # definitions. As well as defining attributes on record instances, the
    # column definitions in {Properties} allow a `CassandraKit::Record` to have a
    # precise internal representation of its representation as a CQL3 table
    ...
    module Schema
      ...
      module ClassMethods
        ...
        #
        # @!attribute [r] columns
        #   (see CassandraKit::Schema::Table#columns)
        #
        # @!attribute [r] column_names
        #   (see CassandraKit::Schema::Table#column_names)
        #
        # @!attribute [r] key_columns
        #   (see CassandraKit::Schema::Table#key_columns)
        #
        # @!attribute [r] key_column_names
        #   (see CassandraKit::Schema::Table#key_column_names)
        #
        # @!attribute [r] partition_key_columns
        #   (see CassandraKit::Schema::Table#partition_key_columns)
        #
        # @!attribute [r] partition_key_column_names
        #   (see CassandraKit::Schema::Table#partition_key_column_names)
        #
        # @!attribute [r] clustering_columns
        #   (see CassandraKit::Schema::Table#clustering_columns)
        #
        # @!method compact_storage?
        #   (see CassandraKit::Schema::Table#compact_storage?)
        #
        ...
        #   (see CassandraKit::Schema::Table#column)
        #
        ...
        def synchronize_schema
          ...
          patch =
            begin
              existing_table_descriptor = CassandraKit::Schema::TableReader.read(connection,
                                                                           table_name)

              return if existing_table_descriptor.materialized_view?

              CassandraKit::Schema::TableDiffer.new(existing_table_descriptor,
                                              table_schema)
                .call

            rescue NoSuchTableError
              CassandraKit::Schema::TableWriter.new(table_schema)
            end
        ...
        def dsl
          @dsl ||= CassandraKit::Schema::TableDescDsl.new(table_name)
        end
