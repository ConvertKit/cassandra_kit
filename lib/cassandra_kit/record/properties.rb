...
module CassandraKit
  module Record
    #
    # Properties on a CassandraKit record acts as attributes on record instances, and
    ...
    #
    # All varieties of column have a type; see {CassandraKit::Type} for the full
    ...
    module Properties
      ...
      module ClassMethods
        ...
        def key(name, type, options = {})
          ...
          if options.fetch(:auto, false)
            unless Type[type].is_a?(CassandraKit::Type::Uuid)
              fail ArgumentError, ":auto option only valid for UUID columns"
            end
            default = -> { CassandraKit.uuid } if options[:auto]
          else
      ...
      def initialize(attributes = {}, record_collection = nil)
        @cassandra_kit_attributes, @record_collection = attributes, record_collection
        @collection_proxies = {}
      end
      ...
      def inspect
        inspected_attributes = attributes.each_pair.map do |attr, value|
          inspected_value = CassandraKit.uuid?(value) ?
            value.to_s :
            value.inspect
      ...
      def read_attribute(name)
        @cassandra_kit_attributes.fetch(name)
      rescue KeyError
      ...
      def write_attribute(name, value)
        ...
        @cassandra_kit_attributes[name] = value
      end
      ...
      def init_attributes(new_attributes)
        ...
        new_attributes.each_pair do |name, value|
          ...
          @cassandra_kit_attributes[name.to_sym] = value
        end
        @cassandra_kit_attributes
      end
