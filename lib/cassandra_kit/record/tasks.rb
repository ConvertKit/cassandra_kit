...
namespace :cassandra_kit do
  namespace :keyspace do
    ...
    task :create_if_not_exist => :environment do
      if CassandraKit::Record.connection.schema.exists?
        puts "Keyspace #{CassandraKit::Record.connection.name} already exists. Nothing to do."
        next
      end
      create!
    end
    ...
    task :drop_if_exist => :environment do
      unless CassandraKit::Record.connection.schema.exists?
        puts "Keyspace #{CassandraKit::Record.connection.name} doesn't exist. Nothing to do."
        next
      end
      drop!
    end
  ...
  def create!
    ...
    puts "Created keyspace #{CassandraKit::Record.connection.name}"
  end


  def drop!
    CassandraKit::Record.connection.schema.drop!
    puts "Dropped keyspace #{CassandraKit::Record.connection.name}"
  end
    ...
    model_files.sort.each do |file|
      ...
      new_constants.each do |class_name|
        ...
        begin
          ...
          if clazz.is_a?(Class)
            if clazz.ancestors.include?(CassandraKit::Record) &&
